(() => {
    ...
    function formRating() {
        ...
        function formRatingSet(rating, value) {
            ...
            if (rating.hasAttribute("data-rating-title")) {
                const previousElement = rating.previousElementSibling;
                let valueRating = null;
                if (previousElement && previousElement.classList.contains("footer__value-rating")) valueRating = previousElement;
                if (valueRating) valueRating.textContent = `${rating.title}`;
            }
    ...
    function initSliders() {
        if (document.querySelector(".slider-news__slider")) new swiper_core_Swiper(".slider-news__slider", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            autoHeight: true,
    ...
    function documentActions(e) {
        ...
        if (targetElement.closest("[data-spoller]")) {
            ...
            if (!currentElement.nextElementSibling.classList.contains("--sliding")) currentElement.classList.toggle("active");
            slideToggle(currentElement.nextElementSibling);
        }
        if (targetElement.closest(".item-product")) {
            const productElement = targetElement.closest(".item-product");
            showPopupProduct(productElement);
        }
    }
    function validation(form) {
        ...
        function removeError(input) {
            ...
            if (parent.classList.contains("error")) {
                const errorElements = parent.querySelectorAll(".form-error");
                errorElements.forEach((errorElement => errorElement.remove()));
                parent.classList.remove("error");
            }
        }
        function createError(input, valueError) {
            const parent = input.parentNode;
            const errorSpan = document.createElement("div");
            errorSpan.classList.add("form-error");
            errorSpan.textContent = valueError;
        ...
        allInputs.forEach((input => {
            ...
            if (input.validity.tooShort) {
                createError(input, `Fields must contains at least ${input.minLength} characters`);
                result = false;
            }
            if (input.validity.tooLong) {
                createError(input, `Fields must contains not more than ${input.maxLength} characters`);
                result = false;
            }
            if (input.validity.patternMismatch) {
                createError(input, input.dataset.errorMessage || "Not correct format");
                result = false;
            }
        }));
        allTextareas.forEach((textarea => {
            ...
            if (textarea.validity.valueMissing) {
                createError(textarea, "Field is empty");
                result = false;
            }
            if (textarea.validity.tooShort) {
                createError(textarea, `Fields must contains at least ${textarea.minLength} characters`);
                result = false;
            }
            if (textarea.validity.tooLong) {
                createError(textarea, `Fields must contains not more than ${textarea.maxLength} characters`);
                result = false;
            }
            if (textarea.validity.patternMismatch) {
                createError(textarea, textarea.dataset.errorMessage || "Not correct format");
                result = false;
            }
        }));
        return result;
    ...
    if (contactForms) contactForms.forEach((contactForm => {
        contactForm.addEventListener("submit", (function(event) {
            event.preventDefault();
            if (validation(this) == true) this.submit();
        }));
    }));
    const script_spollers = document.querySelectorAll("[data-spoller]");
    if (script_spollers.length) {
        const breakpoint = window.matchMedia("(max-width: 767.98px)");
        const updateSpollerText = () => {
            script_spollers.forEach((spoller => {
                if (spoller.classList.contains("filter__title")) if (breakpoint.matches) spoller.textContent = ""; else spoller.textContent = "Filter";
            }));
        };
        updateSpollerText();
        breakpoint.addEventListener("change", updateSpollerText);
        script_spollers.forEach((spoller => {
            const content = spoller.nextElementSibling;
            if (spoller.dataset.spoller === "open") {
    ...
    menuLinks.forEach((menuLink => {
        if (menuLink.href.includes(currentPage)) menuLink.classList.add("menu__link--active");
        if (currentPage.includes("fariman") || currentPage.includes("mahmood") || currentPage.includes("dinnerware")) {
            const menuBrand = Array.from(menuLinks).find((item => item.textContent === "Brand"));
            menuBrand.classList.add("menu__link--active");
        }
    }));
    function showPopupProduct(product) {
        const productImage = product.querySelector(".item-product__image img").getAttribute("src");
        const productName = product.querySelector(".item-product__title").textContent;
        const productDescription = product.querySelector(".item-product__description").textContent;
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        ...
        if (filterForm) filterForm.addEventListener("change", (() => {
            const inputs = filterForm.querySelectorAll("input:checked");
            const selectedFilters = {};
            inputs.forEach((input => {
                const groupName = input.name;
                const value = input.value;
                if (!selectedFilters[groupName]) selectedFilters[groupName] = [];
            ...
            products.forEach((product => {
                ...
                for (const group in selectedFilters) {
                    ...
                    const productAttr = product.dataset.design || "";
                    const hasMatch = values.some((val => productAttr.includes(val)));
                    if (!hasMatch) {
                        isVisible = false;
                        break;
                    }
